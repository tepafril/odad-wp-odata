<?php
/**
 * Integration tests for WPOS_Hook_Bridge and related WP filter/action extension points.
 *
...
class HookBridgeTest extends WP_UnitTestCase {
    ...
    public function setUp(): void {
        parent::setUp();
        // Container is already available via wpos_container() — plugins_loaded
    ...
    /**
     * The 'wpos_register_entity_sets' action hook is registered during init.
     *
    ...
    public function test_wpos_register_entity_sets_fires_on_init(): void {
        // The bridge registers 'wpos_register_entity_sets' on init (priority 1)
        ...
        $this->assertTrue(
            has_action( 'wpos_register_entity_sets' ) !== false,
            "'wpos_register_entity_sets' action must be registered."
        );
    }

    /**
     * Adding a filter on 'wpos_can_read' that returns false causes
     * WPOS_Subscriber_Permission_Check to deny access.
     *
     * We fire the event directly through the event bus so we don't need a
     * full HTTP request.
     */
    public function test_wpos_can_read_filter(): void {
        // Register a filter that denies all reads.
        add_filter( 'wpos_can_read', '__return_false', 99, 3 );
        ...
        try {
            ...
            $event = new WPOS_Event_Permission_Check(
                entity_set: 'Posts',
                operation:  'read',
                user:       $admin_user,
            ...
            $this->assertFalse(
                $event->granted,
                'wpos_can_read returning false must deny permission.'
            );
        } finally {
            remove_filter( 'wpos_can_read', '__return_false', 99 );
        }
    }

    /**
     * A 'wpos_before_insert' filter that modifies the payload is applied before
     * the write is persisted.
     *
     * We dispatch WPOS_Event_Write_Before through the event bus (as the write
    ...
    public function test_wpos_before_insert_filter_modifies_payload(): void {
        ...
        add_filter( 'wpos_before_insert', $filter_fn, 10, 3 );
        ...
        try {
            ...
            $event = new WPOS_Event_Write_Before(
                entity_set: 'Posts',
                operation:  'insert',
                user:       $admin,
            ...
            $this->assertSame(
                ...
                'wpos_before_insert filter must be able to change payload Status to "draft".'
            );
        } finally {
            remove_filter( 'wpos_before_insert', $filter_fn, 10 );
        }
    }

    /**
     * Calling WPOS_Metadata_Cache::bust() deletes both transients so that the

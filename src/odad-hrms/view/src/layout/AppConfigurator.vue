...
const { layoutConfig, isDarkTheme, persistLayoutConfig } = useLayout();
...
function applyStoredTheme() {
    ...
    $t().preset(presets.Lara).preset(getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
}
...
<template>
    <div
        class="config-panel hidden absolute top-[3.25rem] right-0 w-64 p-4 border border-surface rounded-border origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]"

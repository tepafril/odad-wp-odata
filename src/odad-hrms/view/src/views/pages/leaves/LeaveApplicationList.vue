...
import LeaveApplicationForm from './LeaveApplicationForm.vue';
...
function confirmCancel(row) {
    confirm.require({
        message: `Cancel leave application for ${employeeName(row)}?`,
        header: 'Cancel Application',
        icon: 'pi pi-ban',
        acceptClass: 'p-button-danger',
        accept: async () => {
            try {
                await LeaveApplicationService.cancel(row.id);
                toast.add({ severity: 'success', detail: 'Leave application cancelled.', life: 3000 });
                load();
            } catch (e) {
...
<template>
    <div class="card w-full !max-w-full">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
                <div class="font-semibold text-xl mb-1">Leave Applications</div>
                <p class="text-surface-500 m-0">Manage employee leave requests.</p>
            </div>
            <Button label="New Application" icon="pi pi-plus" @click="openNew" />
        </div>
        ...
        <DataTable :value="items" :loading="loading" dataKey="id" lazy paginator
            ...
            <template #empty><div class="text-center py-8 text-surface-500">No leave applications found.</div></template>
                ...
                <template #body="{ data }">
                    <div v-if="data.status === 'pending'" class="flex gap-1">
                        ...
                        <Button icon="pi pi-ban"
                            outlined rounded size="small" severity="danger" @click="confirmCancel(data)" />
                    </div>
                </template>
            </Column>
        </DataTable>
        ...
        <Dialog v-model:visible="dialogVisible"
            :header="editingId ? 'Edit Leave Application' : 'New Leave Application'"
            modal :style="{ width: '46rem' }" :dismissableMask="true" @hide="editingId = null">
            <LeaveApplicationForm ref="formRef" :model-value="dialogVisible" :edit-id="editingId"

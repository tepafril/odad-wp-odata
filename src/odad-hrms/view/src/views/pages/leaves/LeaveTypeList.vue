...
async function load() {
    loading.value = true;
    try {
        const data = await LeaveTypeService.getList({
...
<template>
    <div class="card w-full !max-w-full">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
                <div class="font-semibold text-xl mb-1">Leave Types</div>
                <p class="text-surface-500 m-0">Configure the types of leave available to employees.</p>
            </div>
            <Button label="New Leave Type" icon="pi pi-plus" @click="openNew" />
        </div>
        ...
        <DataTable :value="items" :loading="loading" dataKey="id" lazy paginator
            ...
            <template #empty><div class="text-center py-8 text-surface-500">No leave types found.</div></template>
            ...
            <Column field="name"         header="Name"      sortable style="min-width:10rem">
                <template #body="{ data }">
                    <div class="flex items-center gap-2">
                        <span class="inline-block w-3 h-3 rounded-full" :style="{ background: data.color || '#3B82F6' }"></span>
                        {{ data.name }}
                    </div>
                </template>
            </Column>
            ...
            <Column field="category"     header="Category"  style="min-width:8rem">
                <template #body="{ data }">
                    <Tag :value="data.category || 'general'" :severity="{ general: 'info', sick: 'warn', parental: 'success', compensatory: 'secondary', unpaid: 'danger' }[data.category] || 'info'" class="capitalize" />
                </template>
            </Column>
            <Column field="default_days" header="Days/Year" sortable style="min-width:8rem" />
            <Column field="allow_half_day" header="Half Day" style="min-width:7rem">
                <template #body="{ data }">
                    <i :class="parseInt(data.allow_half_day) ? 'pi pi-check text-green-500' : 'pi pi-times text-red-400'" />
                </template>
            </Column>
            <Column field="carry_forward" header="Carry Fwd" style="min-width:8rem">
                <template #body="{ data }">
                    <i :class="parseInt(data.carry_forward) ? 'pi pi-check text-green-500' : 'pi pi-times text-red-400'" />
                </template>
            </Column>
        ...
        <Dialog v-model:visible="dialogVisible"
            :header="editingId ? 'Edit Leave Type' : 'New Leave Type'"
            modal :style="{ width: '52rem' }" :dismissableMask="true" @hide="editingId = null">
            <LeaveTypeForm ref="formRef" :model-value="dialogVisible" :edit-id="editingId"

...
const allColumns = [
    ...
    { field: 'level', header: 'Level', sortable: true, style: 'min-width:7rem'  },
];
const defaultColumns = ['id', 'name', 'level'];
...
const form = ref({ company_id: null, name: '', level: 1, description: '', status: 'active' });
...
async function load() {
    loading.value = true;
    try {
        const data = await DesignationService.getList({
            page: Math.floor(first.value / rows.value) + 1,
            per_page: rows.value, orderby: sortField.value,
            order: sortOrder.value === 1 ? 'ASC' : 'DESC',
        });
        items.value = data?.items ?? []; totalRecords.value = data?.total ?? 0;
    } catch (e) { toast.add({ severity: 'error', summary: 'Error', detail: e.message, life: 5000 }); }
    finally { loading.value = false; }
}

function resetForm() { Object.assign(form.value, { company_id: null, name: '', level: 1, description: '', status: 'active' }); }
...
async function openEdit(row) {
    ...
    try {
        const d = await DesignationService.getById(row.id);
        Object.assign(form.value, {
            company_id: d.company_id ? Number(d.company_id) : null,
            name: d.name ?? '', level: Number(d.level ?? 1),
            description: d.description ?? '', status: d.status ?? 'active',
        });
    } catch (e) { toast.add({ severity: 'error', summary: 'Error', detail: e.message, life: 5000 }); return; }
    dialogVisible.value = true;
}

function validate() {
    errors.name       = form.value.name?.trim() ? '' : 'Designation name is required.';
...
async function save() {
    ...
    saving.value = true;
    try {
        if (editingId.value) { await DesignationService.update(editingId.value, form.value); toast.add({ severity: 'success', detail: 'Updated.', life: 3000 }); }
        else { await DesignationService.create(form.value); toast.add({ severity: 'success', detail: 'Created.', life: 3000 }); }
...
function confirmDelete(row) {
    confirm.require({
        message: `Delete designation "${row.name}"?`, header: 'Delete', icon: 'pi pi-trash', acceptClass: 'p-button-danger',
        accept: async () => { try { await DesignationService.remove(row.id); toast.add({ severity: 'success', detail: 'Deleted.', life: 3000 }); load(); } catch (e) { toast.add({ severity: 'error', detail: e.message, life: 5000 }); } },
    });
}
...
<template>
    <div class="card w-full !max-w-full">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
                <div class="font-semibold text-xl mb-1">Designations</div>
                <p class="text-surface-500 m-0">Manage job titles and designations.</p>
            </div>
            <div class="flex items-center gap-2">
                ...
                />
                <Button label="New Designation" icon="pi pi-plus" @click="openNew" />
            </div>
        </div>

        <DataTable :value="items" :loading="loading" dataKey="id" lazy paginator
            :first="first" :rows="rows" :totalRecords="totalRecords" :rowsPerPageOptions="[5, 10, 25]"
            paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
            currentPageReportTemplate="Showing {first} to {last} of {totalRecords} records"
            @page="onPage" @sort="onSort">
            <template #empty><div class="text-center py-8 text-surface-500">No designations found.</div></template>
            <Column
        ...
        <Dialog v-model:visible="dialogVisible" :header="editingId ? 'Edit Designation' : 'New Designation'"
            ...
            <div class="flex flex-col gap-4 pt-1">
                <div class="flex flex-col gap-2">
                    ...
                    <Select v-model="form.company_id" :options="companyOptions" optionLabel="name" optionValue="id"
                        placeholder="Select company" fluid :invalid="!!errors.company_id" />
                    <small v-if="errors.company_id" class="text-red-500">{{ errors.company_id }}</small>
                </div>
                <div class="flex flex-col gap-2">
                    <label class="font-medium">Designation Name <span class="text-red-500">*</span></label>
                    <InputText v-model="form.name" fluid :invalid="!!errors.name" />
                    <small v-if="errors.name" class="text-red-500">{{ errors.name }}</small>
                </div>
                <div class="flex flex-col gap-2">
                    <label class="font-medium">Level</label>
                    <InputNumber v-model="form.level" :min="1" fluid />
                </div>
                <div class="flex flex-col gap-2">
